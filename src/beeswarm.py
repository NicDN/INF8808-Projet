...
#  CONFIG & TRADUCTION
# ============================================================
INPUT_CSV   = "rawgraphs_beeswarm.csv"
OUTPUT_HTML = "visualizations/beeswarm_parkinson.html"

COULEUR_PARKINSON = "#F97316"  # Orange
COULEUR_SAIN      = "#3B82F6"  # Bleu
BG_TOOLTIP        = "#262626"  # Fond sombre pour tooltips
...
for task_key in reversed(task_keys):
    ...
    if not subset_sain.empty:
        ...
        hover_sain = [
            ...
            f"<b>Max Tremor Ratio :</b> {row['max_tremor_ratio']:.2f}<br>"
            f"<b>Poignet Droit :</b> {row['ratio_droit']:.2f}<br>"
            f"<b>Poignet Gauche :</b> {row['ratio_gauche']:.2f}"
...
fig.update_layout(
    title=dict(
        text="<b>Comment évolue l'intensité des tremblements selon l'activité ?</b>",
        font=dict(size=22, color="#1F2937"), x=0.5
    ),
    width=FIG_WIDTH,
    ...
    plot_bgcolor="#F8F9FA",
    xaxis=dict(
        title="<b>Intensité du tremblement</b> (0 = aucun, 1 = maximum)",
        range=[-0.05, 1.05],
        gridcolor="white",
        zeroline=False
    ),
    ...
    hoverlabel=dict(bgcolor=BG_TOOLTIP, font=dict(color="white", size=13)),
    legend=dict(
        title_text="Légende :", 
        bordercolor="#DDD", 
        borderwidth=1,
